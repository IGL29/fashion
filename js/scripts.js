...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // buttonSubmitElement.addEventListener('click', (ev) => {
  //   ev.preventDefault();
  ...
  let selectCategoryValue = null;

  const selectSortByOptionElement = document.querySelector('select[name="sort"]');
  selectSortByOptionElement.addEventListener('input', (ev) => {
    selectCategoryValue = ev.currentTarget.value;
    const url = new URL(document.location.href);
    url.searchParams.set(selectSortByOptionElement.name, selectCategoryValue);
    window.history.pushState({}, "", url);
    window.location.reload();
  })

  let selectPricesValue = null;

  const selectSortProductsElement = document.querySelector('select[name="order"]');
  selectSortProductsElement.addEventListener('input', (ev) => {
    selectPricesValue = ev.currentTarget.value;
    const url = new URL(document.location.href);
    url.searchParams.set(selectSortProductsElement.name, selectPricesValue);
    window.history.pushState({}, "", url);
    window.location.reload();
  })

  // Filters
  const checkboxNewElement = document.getElementById('new');
  const checkboxSaleElement = document.getElementById('sale');
  const filtersFormElement = document.getElementById('filters');

  filtersFormElement.addEventListener('submit', (ev) => {
    ev.preventDefault();

    const url = new URL(document.location.href);
    
    if (filterMinPrice !== null) {
      url.searchParams.set('min-price', filterMinPrice);
      url.searchParams.set('max-price', filterMaxPrice);
    }

    url.searchParams.set('new', checkboxNewElement.checked);
    url.searchParams.set('sale', checkboxSaleElement.checked);
  ...
  const wrapperPaginationElement = document.getElementById('paginator');

  wrapperPaginationElement.addEventListener('click', (ev) => {
    console.log(ev.target.tagName)
    if (ev.target.tagName === 'A') {
      console.log('click')
      const url = new URL(document.location.href);
      url.searchParams.set('page', ev.target.dataset.number);
      window.history.pushState({}, "", url);
      window.location.reload();
    }
  })
